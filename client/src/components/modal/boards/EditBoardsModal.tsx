...
import { CreateBoardsForm, IBoardsModalProps } from "../../../interface/modal";
  ...
  useEffect(() => {
    const outsideClickHandle = (event: MouseEvent) => {
      if (
        ...
        boardsModalBtnRef.current &&
        !boardsModalBtnRef.current.contains(event.target as Node)
      ) {
        closeModalHandle();
      }
    };
    if (boardsModal.editBoardsModalOpen) {
      document.addEventListener("mousedown", outsideClickHandle);
    }
    return () => {
      document.removeEventListener("mousedown", outsideClickHandle);
    };
  }, [closeModalHandle, boardsModal, boardsModalBtnRef]);
  ...
  return (
    <div
      className={cls(
        ...
        boardsModal.editBoardsModalOpen ? "" : "hidden"
      )}
    >
